...
from app.skill_matcher import match_skills, normalize_skill
...
def calculate_experience_score(candidate_exp, required_exp):
    ...
    return min(
        1.0,
        max(0.0, candidate_exp / required_exp),
    )
...
def calculate_exact_required_skill_match(
    candidate_skills,
    required_skills,
):
...
def compute_overall_match(
    ...
    exact_required_score = (
        calculate_exact_required_skill_match(
            resume_data.skills or [],
            job_data.required_skills or [],
        )
    )
    ...
    if exact_required_score == 0.0:
        final_score = min(final_score, 29.99)
        recommendation = "Poor Match"

    elif required_score >= 0.90 and final_score >= 90:
    ...
    return {
        ...
        "breakdown": {
            ...
            "exact_required_skills_score": round(
                exact_required_score * 100,
                2,
            ),
